  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, target: "cover" | "body") => {
    ...
    try {
      ...
      if (target === "cover") {
        ...
        // Chèn vào vị trí con trỏ hiện tại trong textarea
        const textarea = textareaRef.current;
        if (textarea) {
          const start = textarea.selectionStart;
          const end = textarea.selectionEnd;
          const insertedText = `\n![Vapor Art](${secureUrl})\n`;
          const newBody = bodyMd.substring(0, start) + insertedText + bodyMd.substring(end);
          setBodyMd(newBody);
          // Hồi phục focus
          setTimeout(() => {
            textarea.focus();
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex flex-col">
            <div className="flex justify-between items-center mb-1">
              <label className="text-xs font-bold uppercase">Nội dung bài viết (Markdown format) (*)</label>
              <div className="flex gap-2">
                ...
                <label
                  ...
                  className="win95-btn text-[10px] font-bold cursor-pointer px-2 py-0.5"
                >
                  🖼️ CHÈN NHANH ẢNH
                </label>
              </div>
            ...
            <textarea
              ...
              rows={12}
              required
              className="p-2 border border-win-dark bg-white outline-none font-mono text-xs shadow-inner leading-relaxed"
              placeholder="# Tiêu đề chính&#10;&#10;Hãy viết bài viết nghệ thuật tại đây sử dụng cú pháp **Markdown** phong phú..."
              value={bodyMd}
